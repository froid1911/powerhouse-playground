...
interface IDocumentDrive {
    addFile: (input: AddFileInput, file: string) => Promise<void>;
    openFile: (path: string, driveId: string) => Promise<string>;
}

interface IElectronDocumentDrive extends IDocumentDrive {
    store: ElectronStore;
    basePath: string;
...
class ElectronDocumentDrive implements IElectronDocumentDrive {
    store: ElectronStore;
    basePath: string;
    document: DocumentDrive;
